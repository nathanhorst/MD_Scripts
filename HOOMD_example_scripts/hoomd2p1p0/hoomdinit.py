...
print type_pos_matrix_by_particle('Au201HC_12_r60.xml','Au')
...
def type_pos_matrix_by_particle_gsd(system,typ):
	tags=[]
	vtags=[]	
	for b in system.particles:
		if b.type==typ:
	...
	for i in range(len(tags)):
		x=system.particles.get(tags[i]).position[0]
		y=system.particles.get(tags[i]).position[1]
		z=system.particles.get(tags[i]).position[2]
		result=np.append(result,[[x,y,z]],axis=0)
	result=result[1:]
	nums=np.zeros(len(vtags))
	for x in range(len(tags)):
		for y in range(len(vtags)):
			if system.particles.get(tags[x]).body==system.particles.get(vtags[y]).body:
				nums[y]+=1
	countdown=np.zeros(len(nums))
	for u in range(len(countdown)):
		countdown[u]=nums[u]
	vs=np.zeros((len(vtags),np.amax(nums),3))
        for x in range(0,len(result)):
	    corrv=get_v(tags[x],vtags,system)
            corrv=int(corrv)
	    print countdown[corrv]
            vs[corrv][nums[corrv]-countdown[corrv]]=result[x]
	    countdown[corrv]-=1  
	return vs,nums

def get_v(part_tag,vtags,system):
	for y in range(len(vtags)):
			if system.particles.get(part_tag).body==system.particles.get(vtags[y]).body:
				return y

def v_pos_matrix_gsd(system):
	tags=[]	
	for b in system.particles:
		if b.type[:1]=='V':
			tags.append(b.tag)
...
def set_up_rigid_gsd(system,sysfile,rigid,distance_scale=1, massAu=14.0424, massS=2.28601):
    tags = []
    #print distance_scale
    au,aunums=type_pos_matrix_by_particle_gsd(system,'Au')
    s,snums=type_pos_matrix_by_particle_gsd(system,'S')
    v=v_pos_matrix_gsd(system)
    for b in system.particles:
        if b.type[:1] == 'V':
            tags.append(b.tag)
    ...
    for i in range(0,len(aunums)):
        x=np.array([[0,0,0]])
        for tr in range(0,int(aunums[i])):
            x=np.append(x,[au[i][tr]],axis=0)
        x=x[1:]
        q=np.array([[0,0,0]])
        for tr in range(0,int(snums[i])):
            q=np.append(q,[s[i][tr]],axis=0)
        q=q[1:]
        g=np.append(x,q,axis=0)
        t=['Au']
